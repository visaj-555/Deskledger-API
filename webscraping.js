...
const scrapeWebsite = async (url) => {
  ...
  try {
    ...
    await sleep(5000); // Wait 5 seconds for the page to load

    // Screenshot for debugging
    await page.screenshot({ path: 'screenshot.png' });

    // Wait for the necessary selector
    ...
    for (const item of data) {
      const existingArea = await AreaPriceModel.findOne({ areaName: item.areaName });

...
const getOverallAnalysis = async (req, res) => {
    try {
        ...
        console.log('FD Match Condition:', fdMatch);
        ...
        // Log match conditions for debugging
        console.log('Gold Match Condition:', goldMatch);
        ...
        const totalInvestedAmount = (fdAnalysis[0]?.totalInvestedAmount || 0) + (goldAnalysis[0]?.totalInvestedAmount || 0);
        const currentReturnAmount = (fdAnalysis[0]?.currentReturnAmount || 0) + (goldAnalysis[0]?.currentReturnAmount || 0);
        const totalReturnAmount = (fdAnalysis[0]?.totalReturnAmount || 0) + (goldAnalysis[0]?.totalReturnAmount || 0);
        const profitAmount = (fdAnalysis[0]?.profitAmount || 0) + (goldAnalysis[0]?.profitAmount || 0);
...
const getCombinedNumAnalysis = async (req, res) => {
    try {
        ...
        const goldAnalysisData = {
            totalInvestedAmountOfGold: Math.round(goldAnalysis[0]?.totalInvestedAmountOfGold || 0),
            currentReturnAmountOfGold: Math.round(goldAnalysis[0]?.currentReturnAmountOfGold || 0),
            totalProfitGainedOfGold: Math.round(goldAnalysis[0]?.totalProfitGainedOfGold || 0),
            userId: userId
        };
            ...
            { $match: match },
            {
                $group: {
                    ...
                    totalProfitGainedOfFds: {
                        $sum: { $subtract: ["$currentReturnAmount", "$totalInvestedAmount"] }
                    }
                }
            }
        ]);
        ...
        const fdAnalysisData = {
            totalInvestedAmountOfFds: Math.round(fdAnalysis[0]?.totalInvestedAmountOfFds || 0),
            currentReturnAmountOfFds: Math.round(fdAnalysis[0]?.currentReturnAmountOfFds || 0),
            totalReturnAmountofFds: Math.round(fdAnalysis[0]?.totalReturnAmountofFds || 0),
            totalProfitGainedOfFds: Math.round(fdAnalysis[0]?.totalProfitGainedOfFds || 0),
            userId: userId
        };
        ...
        res.status(statusCode.OK).json({
            statusCode: statusCode.OK,
            message: "Fetched Combined Analysis of all Fixed Deposits and Golds.",
...
const getHighestGrowthInSector = async (req, res) => {
    ...
    const dateFilters = startDate && endDate ? getDateFilters(startDate, endDate) : {};
        ...
        switch (sector.toLowerCase()) {
            case 'banking':
                highestGrowth = await FixedDepositModel.findOne({
                    sector: 'Banking',
                    userId: req.user.id,
                    ...(Object.keys(dateFilters).length ? { createdAt: dateFilters } : {})
                })
...
const getInvestmentsBySector = async (req, res) => {
    ...
    try {
        ...
        investments = investments.map((item, index) => {
            ...
            console.log("Mapping index:", index, "New srNo:", newItem.srNo);
            return newItem;
        });

        console.log('Final Response Data:', investments);
...
const getInvestmentById = async (req, res) => {
    ...
    try {
        const investmentFD = await FixedDeposit.findOne({ _id: id, userId: req.user.id, createdAt: dateFilters });
        const investmentGold = await GoldModel.findOne({ _id: id, userId: req.user.id, createdAt: dateFilters });
